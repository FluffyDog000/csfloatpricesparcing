...
def date_bounds_iso(
    ...
    """Turn 'YYYY-MM-DD' from/to dates into (since, until) ISO-8601 UTC bounds.
    `since` is the start of from_date; `until` is the end of to_date."""
    since = until = None
    if from_date:
        d = datetime.strptime(from_date, "%Y-%m-%d").replace(tzinfo=timezone.utc)
        since = d.isoformat()
    if to_date:
        d = datetime.strptime(to_date, "%Y-%m-%d").replace(
...
def aggregate_buckets(
    ...
    """Group sales by float bucket; count (= sales in the selected period) and
